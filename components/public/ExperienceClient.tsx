...
export function ExperienceClient({ experiences, skills, hackathons, certifications, settings, education }: ExperienceClientProps) {
  return (
    <div className="relative min-h-screen bg-bg/50 overflow-hidden">
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-6 py-12 lg:py-20">
        {/* Hero */}
        <ScrollReveal animation="fade-up" className="mb-12 grid gap-10 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            ...
            <h1 className="font-display text-4xl tracking-tight sm:text-6xl lg:text-8xl">
              {settings.experienceHeroTitle?.split(" ").slice(0, -1).join(" ")} <br />
              <span className="text-gradient">{settings.experienceHeroTitle?.split(" ").slice(-1).join(" ")}</span>
            </h1>
        ...
        <div className="grid gap-12 lg:grid-cols-12">
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <ScrollReveal animation="fade-up" delay={0.1}>
              ...
              <Card className="overflow-hidden border-border/50 bg-surface/50 backdrop-blur-sm transition-all hover:border-primary/20 shadow-xl">
            ...
            {/* Skills Mosaic */}
            <ScrollReveal animation="fade-up" delay={0.15} className="pt-12">
              <div className="mb-8 px-2">
                <h2 className="font-display text-3xl tracking-tight">Tools <span className="text-gradient">I Use</span></h2>
                <p className="mt-2 text-sm text-muted">A comprehensive look at the tools and frameworks used to build solutions.</p>
              </div>
        ...
        <div className="mt-20 pt-20 border-t border-border/50 space-y-24">
          {/* Hackathons - Immersive Grid */}
          {hackathons.length > 0 && (
            <section>
              <ScrollReveal animation="fade-up" className="mb-12 flex items-end justify-between px-2">
                <div className="max-w-xl">
                  <h2 className="font-display text-3xl tracking-tight lg:text-5xl">Building <span className="text-gradient">Projects.</span></h2>
                  <p className="mt-4 text-muted lg:text-lg">Projects built during hackathons and engineering competitions.</p>
                </div>
              ...
              <StaggerContainer staggerDelay={0.1} className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                {hackathons.map((hackathon) => (
                  <StaggerItem key={hackathon.id}>
                    <Card className="group overflow-hidden border-border/40 bg-surface/20 transition-all duration-500 hover:bg-surface/50 hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-2">
                      {hackathon.image && (
                      ...
                      <div className="p-8">
                        ...
                        <div className="mt-8 flex items-center justify-between pt-6 border-t border-border/50">
                          ...
                          <Link href={`/hackathons/${hackathon.slug}`} className="flex items-center gap-2 text-xs font-bold uppercase tracking-tighter text-primary/80 hover:text-primary transition-all group-hover:gap-3">
          ...
          {certifications.length > 0 && (
            <section>
              <ScrollReveal animation="fade-up" className="mb-12 px-2 text-center lg:text-left">
                <h2 className="font-display text-3xl tracking-tight lg:text-5xl">Certificates.</h2>
                <p className="mt-2 text-muted lg:text-lg">Certificates earned from various courses and professional exams.</p>
              </ScrollReveal>

              <StaggerContainer staggerDelay={0.1} className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {certifications.map((cert) => (
                  <StaggerItem key={cert.id}>
                    <Card className="group relative overflow-hidden border-border/50 bg-surface/30 p-0 transition-all duration-500 hover:border-primary/40 hover:bg-surface/50">
